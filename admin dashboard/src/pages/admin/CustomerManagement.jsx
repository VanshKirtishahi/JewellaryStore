...
const CustomerManagement = () => {
  ...
  const fetchData = async () => {
    try {
      ...
      // Bypass default limits so stats are perfectly accurate
  ...
  const calculateStats = (customersData, ordersData) => {
    ...
    const totalRevenue = ordersData?.reduce((sum, order) => sum + (order?.totalAmount || 0), 0) || 0;
  ...
  const getCustomerMetrics = (userId) => {
    ...
    return { orderCount: custOrders?.length || 0, totalSpent, loyaltyTier };
  };
    ...
    <div className="space-y-8 animate-fadeIn p-6 bg-gray-50 min-h-screen">
      ...
      <div className="bg-white border border-gray-200 rounded-xl shadow-sm overflow-hidden">
        <div className="p-4 border-b border-gray-200 flex flex-col md:flex-row gap-4 items-center justify-between bg-gray-50">
          ...
          <div className="text-sm text-gray-500">Showing <strong>{filteredCustomers?.length || 0}</strong> customers</div>
        </div>
            ...
            <tbody className="divide-y divide-gray-100">
              {filteredCustomers?.length === 0 ? (
                ...
                filteredCustomers?.map((customer) => {
                  ...
                  return (
                    <tr key={customer?._id} className="even:bg-white odd:bg-slate-50 hover:bg-blue-50/50 transition-colors">
                      ...
                      <td className="px-6 py-4 text-right">
                        <div className="flex justify-end gap-2">
                          <button onClick={() => handleViewDetails(customer)} className="p-2 text-gray-500 hover:bg-white rounded-lg hover:text-blue-600 shadow-sm transition-all" title="View Details">
                            <Eye size={16} />
                          </button>
                        </div>
                      </td>
            ...
            <div className="p-6 overflow-y-auto flex-1">
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-8">
                ...
                <div className="bg-emerald-50 p-4 rounded-xl border border-emerald-100">
                  ...
                  <p className="text-2xl font-bold text-emerald-800">{customerOrders?.length || 0}</p>
                </div>
